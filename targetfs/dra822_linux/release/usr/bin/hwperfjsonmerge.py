...
'tdomain' filed (this should help with identification of objects in
merged json file):
  ...
  G2CPU_NTW some synchronisation data was available somewhere before
            this packet but it was lost at some point, so time stamp
            of this packet may be inaccurate
"""
...
class Args(object):
    ...
    def hostjson_fname(self):
        """Returnss dirctionary with Host file names."""
        return self._host
    ...
    def out_fname(self):
        """Returns list of all output files names as a dictianary where the
        key is iDeviceID."""
        return self._out
...
class HwperfFw(Hwperf):
    """Represends HWPerf Firmware stream."""
...
class HwperfClient(Hwperf):
    """Represends HWPerf Client stream."""
...
class Merger(object):
    ...
    def _merge(self, devid):
        if devid in self._syncline:
         ...
         $ echo 0x804000000 > /sys/kernel/debug/pvr/apphint/HWPerfFWFilter
